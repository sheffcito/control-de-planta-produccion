...
datos_produccion = {
    ...
    "produccion": {
        f"Semana {s}": {
            dia: []  # Ahora cada día guarda una lista de registros (por si corres más de una serie al día)
...
def registrar_dia():
    ...
    # NUEVO: Pedir el número o nombre de la serie
    serie = input("Número o Nombre de la Serie (ej. Serie X, Lote 102): ").strip()
    u_hoy = int(input("¿Cuántas unidades se produjeron?: "))
    p_hoy = int(input("¿Cuántos palets completos se armaron para esta serie?: "))
    c_hoy = int(input("¿En qué número de caja quedó el último palet?: "))
    
    # Creamos el registro estructurado
    nuevo_registro = {
        "serie": serie if serie else "Sin Serie",
        "unidades": u_hoy,
        "palets": p_hoy,
        "ultima_caja": c_hoy
    }
    
    # Añadimos el registro al día correspondiente
    datos_produccion["produccion"][semana_sel][dia_sel].append(nuevo_registro)
    
    guardar_datos()
    print(f"\n✅ Registro guardado para {dia_sel} ({serie}).")
    input("\nPresione Enter para volver...")
...
def ver_reporte():
    ...
    print("==================================================")
    print("        REPORTE DETALLADO POR SERIE Y DÍA         ")
    print("==================================================")
    
    for sem, dias in datos_produccion["produccion"].items():
        pauta_sem = datos_produccion["pauta_mensual"][sem]
        
        # Calcular totales de la semana sumando todos los registros de cada día
        real_sem = 0
        palets_sem = 0
        ...
        for dia, registros in dias.items():
            if registros: # Si el día tiene registros en su lista
                print(f"     - {dia}:")
                for reg in registros:
                    print(f"       [Serie: {reg['serie']}] -> {reg['unidades']} u. | {reg['palets']} palets | Última Caja: #{reg['ultima_caja']}")
...
while True:
    limpiar_pantalla()
    print("📱 CONTROL DE PRODUCCIÓN PRO v3.0 (SERIES) 📱")
    print("1. Configurar Pautas Semanales")
    print("2. Registrar Turno (Con Número de Serie)")
